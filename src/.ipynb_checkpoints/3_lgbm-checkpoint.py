...
def optimize_lgb_hyperparameters(X_train, y_train, X_val, y_val, quantile, config):
    def objective(trial):
        ...
        if quantile == 'mean':
            ...
            error = y_val - y_pred
            return np.mean(np.where(error >= 0, float(quantile) * error, (float(quantile) - 1.0) * error))
...
def main():
    ...
    print("=" * 60)
    print(f" LightGBM 兩階段驗證：70% Train (5-Fold OOF) + 30% Test Ensemble")
    print("=" * 60)
    ...
    for fold_idx in range(5):
        ...
        for q in all_quantiles:
            ...
            best_params = optimize_lgb_hyperparameters(X_tr_opt, y_tr_opt, X_va_opt, y_va_opt, q, config)
        ...
        fold_eval_df = evaluate_predictions(y_val_real, fold_preds_real, quantiles, f"LGB_Fold_{fold_idx+1}")
        all_fold_evaluations.append(fold_eval_df)
        
        save_detailed_predictions(
            fold_predictions, y_val_real, idx_val, raw_data_path, 
            os.path.join(result_dir, f'{target_year}_lgb_{EXP_LEVEL}_fold_{fold_idx+1}_predictions.csv')
        )
    ...
    save_detailed_predictions(
        ...
        os.path.join(result_dir, f'{target_year}_lgb_{EXP_LEVEL}_oof_predictions.csv')
    )
    
    oof_eval = evaluate_predictions(y_train_real_million, oof_preds_real, quantiles, "LightGBM_Train_70_OOF")
    oof_eval['Execution_Time'] = exec_time_str
    train_report_df = pd.concat(all_fold_evaluations + [oof_eval], ignore_index=True)
    train_report_df.to_csv(os.path.join(eval_dir, f'{target_year}_lgb_{EXP_LEVEL}_5fold_evaluation.csv'), index=False)
    ...
    save_detailed_predictions(
        ...
        os.path.join(result_dir, f'{target_year}_lgb_{EXP_LEVEL}_test_predictions.csv')
    )
    
    # ★ 修復點：直接承接 evaluate_predictions 回傳的 DataFrame
    test_report_df = evaluate_predictions(y_test_real_million, test_preds_real, quantiles, "LightGBM_Test_30_Ensemble")
    test_report_df['Execution_Time'] = exec_time_str
    test_report_df.to_csv(os.path.join(eval_dir, f'{target_year}_lgb_{EXP_LEVEL}_test_evaluation.csv'), index=False)

    # ---------- [在終端機印出最終成績單] ----------
    print("\n" + "=" * 80)
    print(f" LightGBM 最終成績單 - {EXP_LEVEL} (單位：百萬元)")
    print("=" * 80)
